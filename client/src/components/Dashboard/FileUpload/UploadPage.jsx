...
const FileUploader = ({ onUploadSuccess }) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (uid) {
        await dispatch(getUserFiles(uid));
      }
      onUploadSuccess?.();
    } catch (err) {
      ...
      {loading && (
        <div className="absolute inset-0 bg-black/30 flex items-center justify-center z-10 rounded-lg">
          <div className="flex flex-col items-center gap-3 bg-white dark:bg-gray-800 px-8 py-6 rounded-lg shadow-lg">
            ...
            <p className="font-medium text-[var(--text-color)]">Uploading files...</p>
          </div>
        </div>
      )}
      ...
      {files.length === 0 && uploadedFiles.length === 0 ? (
        ...
        <div className="file-previews">
          {files.map((file, index) => (
            <div className="file-preview" key={`pending-${index}`}>
          ...
          {uploadedFiles.map((file) => (
            <div className="file-preview border-2 border-green-400" key={file._id}>
              <div className="preview-img-container">
                <div className="file-icon">✅</div>
              </div>
              <div className="file-info">
                <div className="file-name" title={file.name}>{file.name}</div>
                <div className="file-size text-green-600">Uploaded successfully</div>
              </div>
            </div>
          ))}
        </div>
